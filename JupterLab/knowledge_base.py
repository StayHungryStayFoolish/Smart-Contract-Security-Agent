...
valid_embedding_models = ["amazon.titan-embed-text-v2:0", "amazon.titan-embed-text-v1", "cohere.embed-multilingual-v3", "cohere.embed-english-v3"]
...
class BedrockKnowledgeBase:
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_model_id (str): embedding model to use
        ...
        if data_bucket_name is not None:
            self.kb_bucket_name = data_bucket_name
        else:
            self.kb_bucket_name = f"{self.kb_name}-{self.suffix}"
        self.schema_bucket_name = schema_bucket_name
        self.encryption_policy_name = f"bedrock-smart-contract-audit-rag-sp-{self.suffix}"
        self.network_policy_name = f"bedrock-smart-contract-audit-rag-np-{self.suffix}"
        self.access_policy_name = f'bedrock-smart-contract-audit-rag-ap-{self.suffix}'
        ...
        self.vector_store_name = f'bedrock-smart-contract-audit-rag-{self.suffix}'
        self.index_name = f"bedrock-smart-contract-audit-rag-index-{self.suffix}"
        print("========================================================================================")
        print(f"Embedding Model ID : {self.embedding_model_id}")
        print(f"Step 1 - Creating or retrieving {self.kb_bucket_name} S3 bucket for Knowledge Base documents")
    ...
    def create_s3_bucket(self):
        ...
        """
        try:
            self.s3_client.head_bucket(Bucket=self.kb_bucket_name)
            print(f'Bucket {self.kb_bucket_name} already exists - retrieving it!')
        except ClientError as e:
            print(f'Creating bucket {self.kb_bucket_name}')
            if self.region_name == "us-east-1":
                self.s3_client.create_bucket(
                    Bucket=self.kb_bucket_name
                )
            else:
                self.s3_client.create_bucket(
                    Bucket=self.kb_bucket_name,
            ...
            "Statement": [
                {
                    ...
                    "Resource": [
                        f"arn:aws:bedrock:{self.region_name}::foundation-model/{self.embedding_model_id}"
                    ]
                }
            ]
        }
            ...
            "Statement": [
                {
                    ...
                    "Resource": [
                        f"arn:aws:s3:::{self.kb_bucket_name}",
                        f"arn:aws:s3:::{self.kb_bucket_name}/*",
                        f"arn:aws:s3:::{self.schema_bucket_name}",
                        f"arn:aws:s3:::{self.schema_bucket_name}/*",
    ...
    def create_vector_index(self):
        """
        Create OpenSearch Serverless vector index for smart contract knowledge. If existent, ignore
        """
        body_json = {
            "settings": {
                "index.knn": "true",
        ...
        try:
            ...
            print('\nCreating smart contract knowledge index:')
            pp.pprint(response)
    ...
    def create_knowledge_base(self):
        ...
        s3_configuration = {
            "bucketArn": f"arn:aws:s3:::{self.kb_bucket_name}",
            # "inclusionPrefixes":["*.*"] # you can use this if you want to create a KB using data within s3 prefixes.
        }

        # The embedding model used by Bedrock to embed ingested documents, and realtime prompts
        embedding_model_arn = f"arn:aws:bedrock:{self.region_name}::foundation-model/{self.embedding_model_id}"
        try:
    ...
    def get_bucket_name(self):
        ...
        """
        pp.pprint(f"Bucket connected with KB: {self.kb_bucket_name}")
        return self.kb_bucket_name
    ...
    def delete_s3(self):
        ...
        """
        objects = self.s3_client.list_objects(Bucket=self.kb_bucket_name)
        if 'Contents' in objects:
            for obj in objects['Contents']:
                self.s3_client.delete_object(Bucket=self.kb_bucket_name, Key=obj['Key'])
        self.s3_client.delete_bucket(Bucket=self.kb_bucket_name)


        objects = self.s3_client.list_objects(Bucket=self.schema_bucket_name)
        if 'Contents' in objects:
            for obj in objects['Contents']:
                self.s3_client.delete_object(Bucket=self.schema_bucket_name, Key=obj['Key'])
        self.s3_client.delete_bucket(Bucket=self.schema_bucket_name)